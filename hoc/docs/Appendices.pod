...
HOC is distributed as a standard UNIX tarball (F<.tar.bz2> file), and
uses GNU I<autoconf> for its build system.  Building HOC should
be a simple matter of the standard autoconf build mantra:
...
Building HOC from a CVS repository is only slightly more
involved: see the F<BUILDING.CVS> file in your checked out CVS
directory (which is not included in proper release tarball) for
more information.
...
scratch using many of the ideas from Mocha and AndrE<acute>'s
